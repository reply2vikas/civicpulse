...
Uses Gemini when GOOGLE_API_KEY (or GEMINI_API_KEY) is set; otherwise falls
back to a deterministic template so the app always renders.
"""
...
def _fallback(hotspots: pd.DataFrame, kpi: dict) -> str:
    ...
    for i, r in enumerate(rows.itertuples(), 1):
        flag = " ⚠️ public-safety" if r.severity >= 4 else ""
        out.append(
    ...
    out.append("\n_(Template brief — add a Gemini API key for AI-generated guidance.)_")
    return "\n".join(out)
...
def generate_brief(hotspots: pd.DataFrame, kpi: dict) -> str:
    ...
    try:
        ...
        return _fallback(hotspots, kpi) + f"\n\n_(Gemini error: {e})_"
